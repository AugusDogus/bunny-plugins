...
const buildLogger = (name) => ({
  ...
  buildEnd: (error) => {
    ...
    const log = (text, color) => {
      ...
      console.log(
        `${ansi.gray}[${time}]${ansi.reset} ${color}${text}${ansi.reset}`
      );
    };
...
const manifestWriter = (manifest) => ({
  ...
  buildEnd: async () => {
    const repo = JSON.parse(
      existsSync('./dist/repo.json')
        ? (await readFile('./dist/repo.json')).toString()
        : '{}'
    );
    ...
    await writeFile(
      `./dist/builds/${manifest.id}/manifest.json`,
...
for (const plug of await readdir('./plugins')) {
  const manifest = JSON.parse(
    (await readFile(`./plugins/${plug}/manifest.json`)).toString()
  );
